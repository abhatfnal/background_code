...
//    Double_t tracklength_det;
    ...
    Double_t muonStartX_det=0.0;
    Double_t muonStartY_det=0.0;
    Double_t muonStartZ_det=0.0;
    Double_t muonEndX_det=0.0;
    Double_t muonEndY_det=0.0;
    Double_t muonEndZ_det=0.0;
    
    
    
    Double_t muontracklength;
    Double_t muontracklength_det=0.0;
    ...
    Double_t StartX_reco=0.0;
    Double_t StartY_reco=0.0;
    Double_t StartZ_reco=0.0;
    Double_t EndX_reco=0.0;
    Double_t EndY_reco=0.0;
    Double_t EndZ_reco=0.0;
    
    Double_t tracklength_reco=0.0;
    
    Double_t dotprod=0.0;
    Double_t costheta=0.0;
    Double_t angle=0.0;
    Double_t angle_smallest=0.0;
  ...
  //  Int_t recotrackcount_smallest=0;
    Double_t tracklength_reco_smallest=0.0;
    Double_t dotprod_smallest=0.0;
    Double_t StartX_reco_smallest=0.0;
    Double_t StartY_reco_smallest=0.0;
    Double_t StartZ_reco_smallest=0.0;
    Double_t EndX_reco_smallest=0.0;
    Double_t EndY_reco_smallest=0.0;
    Double_t EndZ_reco_smallest=0.0;
    Double_t costheta_smallest=0.0;
    Double_t tracklength_diff=0.0; //reco-muon
    Double_t tracklength_ratio=0.0; //(reco/muon)
    ...
    //Match of muon track with best reco track branch
    
 //   TTree* MCTracktree = new TTree("MatchTracktree","MatchTracktree");
          ...
          if (mypdg==13 || mypdg==-13){
              ...
              for (auto const& b : recotrack_vec){
               ...
               //   recotrackcount++;
                ...
                angle = acos(costheta);
                //angle=(acos(((abs(((muonEndX_det-muonStartX_det)*(EndX_reco-StartX_reco))+((muonEndY_det-muonStartY_det)*(EndY_reco-StartY_reco))+((muonEndZ_det-muonStartZ_det)*(EndZ_reco-StartZ_reco))))/(muontracklength_det*tracklength_reco))))*(180.0/M_PI);
              ...
              for (auto const& b : recotrack_vec){
                  ...
                  //muontrackcount_smallest = muontrackcount;
                   ...
                   /*
                       Double_t tracklength_muon_smallest=muontracklength_det;
                       Double_t StartX_muon_smallest=muonStartX_det;
                       Double_t StartY_muon_smallest=muonStartY_det;
                       Double_t StartZ_muon_smallest=muonStartZ_det;
                       Double_t EndX_muon_smallest=muonEndX_det;
                       Double_t EndY_muon_smallest=muonEndY_det;
                       Double_t EndZ_muon_smallest=muonEndZ_det;

                      */
                       
                       
                       
                //   cout<<"Smallest Angle: "<<smallest<<endl;
                //       cout<<"Reco track length for smallest angle: "<<tracklength_reco_smallest<<endl;
                //       cout<<"MC muon track length for smallest angle: "<<muontracklength_det<<endl;
               //        cout<<"Dot prod for smallest: "<<dotprod_smallest<<endl;
              //     cout<<"muontrackcount for smallest: "<<muontrackcount<<endl;
              //     cout<<"recotrackcount for smallest: "<<recotrackcount<<endl;
              ...
              cout<<"Best Match Angle in degrees: "<<angle_smallest<<endl;
...
//      cout<<"muontracklength_det: "<<muontracklength_det<<endl;
      
 //     cout<<"muontrackcount: "<<muontrackcount<<endl;
      
 //     cout<<"tracklength_reco: "<<tracklength_reco<<endl;
      
    //This is where it all begins
  /*
      Int_t nentries = muontrackcount;
      Int_t mentries = recotrackcount;
      cout <<"nentries: "<<nentries<<endl;
      cout <<"mentries: "<<mentries<<endl;
      vector<vector<Double_t>> dotprod;
      vector<vector<Double_t>> costheta;
      vector<vector<Double_t>> angle;
      vector<vector<Double_t>> smallest;
      dotprod.resize(nentries);
      costheta.resize(nentries);
      angle.resize(nentries);
      smallest.resize(nentries);
      
      Int_t counter1=0;
      Int_t counter2=0;
      
      Int_t counter_mctracks=0;
      cout<<"poop1"<<endl;
      for (Int_t i=0;i<nentries;i++) {
          counter_mctracks++;
          dotprod[i].resize(mentries);
          costheta[i].resize(mentries);
          angle[i].resize(mentries);
          smallest[i].resize(mentries);
          cout<<"counter_mctracks: "<<counter_mctracks<<endl;
          Int_t counter_recotracks=0;
          for (Int_t j=0;j<mentries;j++) {
    
              counter_recotracks++;
              cout<<"counter_recotracks: "<<counter_recotracks<<endl;
    
             
         //     dotprod[i][j]=abs(((muonEndX_det-muonStartX_det)*(EndX_reco-StartX_reco))+((muonEndY_det-muonStartY_det)*(EndY_reco-StartY_reco))+((muonEndZ_det-muonStartZ_det)*(EndZ_reco-StartZ_reco)));
              
         //     costheta[i][j]=((dotprod[i][j])/(muontracklength_det*tracklength_reco));
              
              angle[i][j]=(acos(((abs(((muonEndX_det-muonStartX_det)*(EndX_reco-StartX_reco))+((muonEndY_det-muonStartY_det)*(EndY_reco-StartY_reco))+((muonEndZ_det-muonStartZ_det)*(EndZ_reco-StartZ_reco))))/(muontracklength_det*tracklength_reco))))*(180.0/M_PI);
              
           //   smallest[i][j]=2.0;
                 cout<<"Angle in Degrees: "<<angle[i][j]<<endl;
              //  cout<<"Counter1: "<<counter1<<endl;
              //   cout<<"Counter2: "<<counter2<<endl;
         //    for(Int_t i=0;i<nentries;i++){
            //      if(angle[i][j]<smallest[i][j]){
            //          smallest[i][j]=angle[i][j];
                  //    counter_tracks++;
                  //    counter_mctracks++;
                   //   cout<<"Counter_mctracks: "<<counter_mctracks<<endl;
                   //   cout<<"Counter1: "<<counter1<<endl;
                   //   cout<<"Counter2: "<<counter2<<endl;
                    //  cout<<"Smallest value of Angle in degrees: "<<smallest[i][j]<<endl;
                      //cout<<"Tracks Found: "<<counter_tracks<<endl;
