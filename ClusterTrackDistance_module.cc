...
void ClusterTrackDistance::analyze(art::Event const & e)
{
  ...
  for (size_t i_c = 0, size_cluster = cluster_handle->size(); i_c != size_cluster; ++i_c) { //START CLUSTER FOR LOOP
    ...
    //  auto const& cluster = cluster_handle->at(i_c);
    ...
    if(cluster[i_c].View()==2){//START CLUSTER IF LOOP for PLANE MATCHING

      //    cout<<"This cluster is from Plane2"<<endl;
      start_tick2= cluster[i_c].StartTick ();
      end_tick2= cluster[i_c].StartTick ();
    }//END CLUSTER IF LOOP for PLANE MATCHING

    if(cluster[i_c].View()==1){//START CLUSTER IF LOOP for PLANE MATCHING

      //    cout<<"This cluster is from Plane2"<<endl;
      start_tick1= cluster[i_c].StartTick ();
      end_tick1= cluster[i_c].StartTick ();
    }//END CLUSTER IF LOOP for PLANE MATCHING

    if(cluster[i_c].View()==0){//START CLUSTER IF LOOP for PLANE MATCHING

      //    cout<<"This cluster is from Plane2"<<endl;
      start_tick0= cluster[i_c].StartTick ();
      end_tick0= cluster[i_c].StartTick ();
    }//END CLUSTER IF LOOP for PLANE MATCHING
